...
$(function() {
	...
	$("#content").css("width", $loveHeart.width() +
		$("#code").width());
	$("#content").css("height", Math.max(
		$loveHeart.height(), $("#code").height()));
	$("#content").css("margin-top", Math.max((
			$window.height() - $("#content").height()) /
		2, 10));
	$("#content").css("margin-left", Math.max((
...
$(window).resize(function() {
	...
	if (newWidth != clientWidth && newHeight !=
		clientHeight) {
...
function getHeartPoint(angle) {
	...
	var y = -20 * (13 * Math.cos(t) - 5 * Math.cos(2 *
		t) - 2 * Math.cos(3 * t) - Math.cos(4 * t));
...
function startHeartAnimation() {
	...
	var animationTimer = setInterval(function() {
		...
		for (var i = 0; i < heart.length; i++) {
			var p = heart[i];
			var distance = Math.sqrt(Math.pow(p[0] -
				bloom[0], 2) + Math.pow(p[1] - bloom[1],
				2));
			if (distance < Garden.options.bloomRadius.max *
				1.3) {
				draw = false;
				break;
			}
...
function timeElapse(date) {
	var current = Date();
	var seconds = (Date.parse(current) - Date.parse(
		date)) / 1000;
...
function adjustCodePosition() {
	$('#code').css("margin-top", ($("#garden").height() -
		$("#code").height()) / 2);
}
