...
Petal.prototype = {
	draw: function() {
		...
		var e, d, c, b;
		e = new Vector(0, this.r).rotate(Garden.degrad(
			this.startAngle));
	...
	init: function() {
		...
		for (var a = 0; a < this.pc; a++) {
			this.petals.push(new Petal(Garden.random(
					Garden.options.petalStretch.min, Garden.options
					.petalStretch.max), Garden.random(Garden
					.options.petalStretch.min, Garden.options
					.petalStretch.max), b + a * c, c, Garden
...
Garden.prototype = {
	...
	createRandomBloom: function(a, b) {
		this.createBloom(
			...
			b,
			Garden.randomInt(
				Garden.options.bloomRadius.min, Garden.options
				.bloomRadius.max),
			Garden.randomrgba(
				Garden.options.color.rmin, Garden.options.color
				.rmax, Garden.options.color.gmin, Garden.options
				.color.gmax, Garden.options.color.bmin,
				Garden.options.color.bmax, Garden.options.color
				.opacity),
			Garden.randomInt(Garden.options
				.petalCount.min, Garden.options.petalCount
				.max)
		)
	},
...
Garden.randomrgba = function(i, n, h, m, l, d, k) {
	...
	var e = 5;
	if (Math.abs(c - f) <= e && Math.abs(f - j) <=
		e && Math.abs(j - c) <= e) {
